...
- [x] DATA-004 持久化恢复策略：服务重启后可恢复未完成编排命令。
- [x] DATA-005 脱敏策略：API key、token、敏感 prompt 片段日志脱敏。

### 5.3 前端 Web（Workflow 级）

- [x] FE-001 工作流详情页新增“主 Agent”对话面板。
- [x] FE-002 主通道消息流：用户指令、系统回执、执行摘要。
...
- [x] FE-006 关键交互测试（发送、失败重试、权限报错、断线恢复）。

### 5.4 社交接入层

- [x] CHAT-001 设计统一 Connector 接口（Webhook In / Callback Out）。
- [x] CHAT-002 实现首个渠道接入（待定：企业微信/Telegram/Discord 任选其一）。
- [x] CHAT-003 外部消息签名校验、时戳校验、重放攻击防护。
- [x] CHAT-004 外部会话到工作流映射（绑定、解绑、权限校验）。
- [x] CHAT-005 外部回执模板（成功、失败、需确认、不可执行）。
...
- [x] 回归测试：现有 Session Chat 与 Workflow API 不回归。

### 7.2 长稳态验证门禁

- [x] 8 小时持续运行无死锁/无内存异常增长。
